...
if __name__ == '__main__':
    ...
    parser.add_argument('--device', type=str, default='', help='inference device: cpu, cuda, or mps')
    args = parser.parse_args()

    requested = (args.device or os.environ.get('BADMINTON_DEVICE') or '').strip().lower()
    if requested in ('', 'auto'):
        requested = 'cuda' if torch.cuda.is_available() else ('mps' if getattr(torch.backends, 'mps', None) and torch.backends.mps.is_available() else 'cpu')
    if requested.startswith('cuda') and not torch.cuda.is_available():
        raise RuntimeError('请求使用 CUDA，但当前 PyTorch/CUDA 不可用；请设置 BADMINTON_DEVICE=cpu 或 mps')
    if requested == 'mps' and not (getattr(torch.backends, 'mps', None) and torch.backends.mps.is_available()):
        raise RuntimeError('请求使用 MPS，但当前环境不可用；请设置 BADMINTON_DEVICE=cpu')
